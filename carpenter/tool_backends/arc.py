...
_PLAN_FIELDS = (
    ...
    "model_policy_id", "created_at", "updated_at",
)
    ...
    with db_transaction() as db:
        try:
            ...
            for idx, arc_spec in enumerate(arcs_list):
                ...
                # the open transaction rather than tripping the get_db()
                # transaction guard.
            ...
            db.rollback()
            logger.exception("Batch arc creation failed (parent_id=%s)", parent_id)
            return {"error": str(e)}
