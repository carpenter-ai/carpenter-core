...
def test_legacy_forgejo_url_yaml_migrated_to_git_url(tmp_path):
    """Old forgejo_url in YAML is rewritten in-place to git_url on load."""
    yaml_file = tmp_path / "config.yaml"
    ...
    config = load_config(yaml_path=str(yaml_file))
    assert config["git_url"] == "https://forge.example.com"
    # Migration is one-shot and writes back to disk.  Check that the YAML key
    # itself (line-anchored) was rewritten rather than searching for the
    # substring (tmp_path itself may contain "forgejo_url").
    import yaml as yaml_mod
    rewritten = yaml_mod.safe_load(yaml_file.read_text())
    assert "forgejo_url" not in rewritten
    assert rewritten["git_url"] == "https://forge.example.com"


def test_legacy_git_server_url_yaml_migrated_to_git_url(tmp_path):
    """Intermediate git_server_url name in YAML is also migrated to git_url."""
    yaml_file = tmp_path / "config.yaml"
    ...
    import yaml as yaml_mod
    rewritten = yaml_mod.safe_load(yaml_file.read_text())
    assert "git_server_url" not in rewritten
    assert rewritten["git_url"] == "https://forge.example.com"


def test_canonical_key_wins_when_both_present(tmp_path):
    """When both legacy and canonical keys are present, canonical is kept."""
    yaml_file = tmp_path / "config.yaml"
    yaml_file.write_text(
        f"base_dir: {tmp_path}\n"
        "forgejo_url: https://old.example.com\n"
        "git_url: https://new.example.com\n"
    )

    from carpenter.config import load_config

    config = load_config(yaml_path=str(yaml_file))
    assert config["git_url"] == "https://new.example.com"
    import yaml as yaml_mod
    rewritten = yaml_mod.safe_load(yaml_file.read_text())
    assert "forgejo_url" not in rewritten
    assert rewritten["git_url"] == "https://new.example.com"


def test_legacy_forgejo_token_in_dot_env_migrated(tmp_path):
    """FORGEJO_TOKEN in {base_dir}/.env is rewritten to GIT_TOKEN on load."""
    yaml_file = tmp_path / "config" / "config.yaml"
    yaml_file.parent.mkdir(parents=True)
    yaml_file.write_text(f"base_dir: {tmp_path}\n")
    dot_env = tmp_path / ".env"
    dot_env.write_text("FORGEJO_TOKEN=legacy-tok\n")

    from carpenter.config import load_config

    config = load_config(yaml_path=str(yaml_file))
    assert config["git_token"] == "legacy-tok"
    rewritten = dot_env.read_text()
    assert "FORGEJO_TOKEN" not in rewritten
    assert "GIT_TOKEN=legacy-tok" in rewritten
