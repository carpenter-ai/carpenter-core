...
class TestSelfEscalation:
    ...
    def test_escalate_creates_stronger_sibling(self):
        ...
        config_id = arc_manager.get_or_create_model_policy(model="anthropic:claude-haiku-4-5")
        child = arc_manager.add_child(parent, "worker", model_policy_id=config_id)
        arc_manager.update_status(child, "active")

        with patch("carpenter.agent.model_resolver.get_next_model", return_value="anthropic:claude-sonnet-4-6"):
            result = _execute_chat_tool("escalate", {}, executor_arc_id=child)

        assert "Escalated to anthropic:claude-sonnet-4-6" in result
        assert "Arc #" in result

    def test_escalate_freezes_original(self):
        """Original status is 'escalated'."""
        config_id = arc_manager.get_or_create_model_policy(model="anthropic:claude-haiku-4-5")
        arc_id = _create_arc("worker", model_policy_id=config_id)
    ...
    def test_escalate_enhanced_goal(self):
        ...
        parent = _create_arc("parent")
        config_id = arc_manager.get_or_create_model_policy(model="anthropic:claude-haiku-4-5")
        child = arc_manager.add_child(parent, "worker", goal="Fix the login bug", model_policy_id=config_id)
        arc_manager.update_status(child, "active")
    ...
    def test_escalate_at_top_tier_returns_error(self):
        ...
        config_id = arc_manager.get_or_create_model_policy(model="anthropic:claude-opus-4-7")
        arc_id = _create_arc("worker", model_policy_id=config_id)
        arc_manager.update_status(arc_id, "active")
    ...
    def test_escalate_preserves_priority(self):
        """The escalated sibling arc inherits the original arc's priority
        (not just the parent's default). Uses non-100 priority to verify
        real inheritance rather than default coincidence."""
        parent = _create_arc("parent", priority=100)
        config_id = arc_manager.get_or_create_model_policy(model="anthropic:claude-haiku-4-5")
        # Child arc is created with an explicit non-default priority that
        # differs from the parent's — if _handle_escalate only inherited
        # from the parent, this test would fail.
        child = arc_manager.add_child(
            parent, "worker", model_policy_id=config_id, priority=17,
        )
        arc_manager.update_status(child, "active")
