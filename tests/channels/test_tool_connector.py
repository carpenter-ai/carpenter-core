...
class TestFileWatchToolConnector:
    ...
    async def test_start_creates_folder(self, tmp_path):
    ...
    async def test_health_check_no_heartbeat(self, tmp_path):
        shared = tmp_path / "shared"
        shared.mkdir()
        c = FileWatchToolConnector("test", {
            "enabled": True,
    ...
    async def test_health_check_with_heartbeat(self, tmp_path):
        shared = tmp_path / "shared"
        shared.mkdir()
    ...
    async def test_health_check_disabled(self, tmp_path):
        c = FileWatchToolConnector("test", {
            "enabled": False,
            "shared_folder": str(tmp_path),
        })
