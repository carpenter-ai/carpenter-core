...
- **dispatch-actions** (Python) fans structured `proposed_actions` into child arcs via `handle_invoke_coding_change`. Each action's `target_path` drives workflow selection directly — an existing KB path routes to `kb-change` on that entry, a new path routes to a fresh `kb-change` / `coding-change` per `select_workflow_for_paths`. Actions whose `target_path` looks like a per-time-period diary entry (`reflections/*`, dated components, etc.) are dropped as a safety net against LLM regression; drops are recorded on `_dispatch_dropped_diary_targets` for provenance.
